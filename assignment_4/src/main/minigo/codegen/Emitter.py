...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is IntType:
            ...
            lenDim = len(inType.dimens) if inType.dimens else 1
            return "[" * lenDim + self.getJVMType(inType.eleType)
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isStatic, isFinal, value):
        ...
        #isFinal: Boolean
        #value: String
        typeJVM = self.getJVMType(in_)
        if isStatic:
            return self.jvm.emitSTATICFIELD(lexeme, typeJVM, isFinal, value)
        else:
            return self.jvm.emitINSTANCEFIELD(lexeme, typeJVM, isFinal, value)
        
    def emitINSTANCEFIELD(self, lexeme, in_):
        #lexeme: String
        #in_: Type
        return self.jvm.emitINSTANCEFIELD(lexeme, self.getJVMType(in_), False, None).replace(".field",".field public")
    ...
    * @param in the type descriptor of the method.
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, frame):
        ...
        frame.pop() 
        if not type(in_.rettype) is VoidType:
            frame.push() 
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is cgen.ArrayType or type(in_) is cgen.ClassType or type(in_) is Id:
            return self.jvm.emitARETURN()
    ...
    *   .super java/lang/Object<p>
    '''
    def emitPROLOG(self, name, parent, isIn=False):
        #name: String
        #parent: String

        result = list()
        result.append(self.jvm.emitSOURCE(name + ".java"))
        result.append(self.jvm.emitCLASS(f"public {"interface" if isIn else ""} {name}"))
        result.append(self.jvm.emitSUPER("java/lang/Object" if parent == "" else parent))
        return ''.join(result)
    
    def emitSTRUCT(self, name, isIn=False):
        #name: String
        #parent: String
        ...
        return ''.join(result).replace(".field",".field public")
    ...
    def emitANEWARRAY(self, in_, frame):
        frame.push()
        return self.jvm.emitANEWARRAY(self.getJVMType(in_) if type(in_) is ArrayType else {"Ljava/lang/String;": "java/lang/String", "[I": "[I", "[Z": "[Z", "[F": "[F", "[Ljava/lang/String;": "[java/lang/String"}.get(self.getJVMType(in_), self.getJVMType(in_).lstrip("L").rstrip(";")))

    def emitMULTIANEWARRAY(self, in_, num, frame):
        list(map(lambda x: frame.pop(), range(num)))
        frame.push()
        return self.jvm.emitMULTIANEWARRAY(in_, num)
